...
CheckableProxyModel::CheckableProxyModel(QObject *parent)
    ...
    , m_cleanupTimer(new DelayedExecutionTimer(30000,
                                               1000,
                                               this))
...
QVariant CheckableProxyModel::data(const QModelIndex &index,
                                   int role) const
{
    QModelIndex sourceIndex = mapToSource(index);
...
bool CheckableProxyModel::setData(const QModelIndex &index,
...
Qt::CheckState CheckableProxyModel::resolveCheckStateRole(QModelIndex sourceIndex) const
{
    ...
    case CheckableProxyModel::DeterminedByParent: {
        ...
        if (parentState.nodeState == CheckableProxyModel::DeterminedByChildren) {
            return (parentState.defaultChildState == CheckableProxyModel::Checked ? Qt::Checked : Qt::Unchecked);
        }
...
CheckableProxyModel::NodeState CheckableProxyModel::getTreeNodeState(QModelIndex sourceIndex) const
{
    ...
    return m_checkStates.value(pIndex,
                               NodeState(CheckableProxyModel::DeterminedByParent, m_baseState.defaultChildState));
}
    ...
    if (rows > 1) {
        for (int i = 1; i < rows; ++i) {
            ...
            case CheckableProxyModel::DeterminedByParent: {
                ...
                default:
                    qWarning()
                        << "Inconsistency in checkstates!"
                        << __LINE__
                        << sourceModel()->index(i, 0, sourceIndex)
                        << sourceModel()->index(i, 0, sourceIndex).data(Qt::DisplayRole).toString();
                    return Qt::PartiallyChecked;
                }
                break;
            }
...
bool CheckableProxyModel::setCheckState(QModelIndex sourceIndex,
                                        Qt::CheckState state)
{
#ifdef CHECKABLEPROXYMODEL_DEBUG
    ...
    forever {
        ...
        qDebug()
            << "updated node"
            << currentParent
            << currentParent.data(Qt::DisplayRole)
            << parentNodeState.nodeState
            << parentNodeState.defaultChildState;
#endif
...
void CheckableProxyModel::removeSubtree(QModelIndex sourceIndex)
{
    ...
    if (rowCount > 0)
        emit dataChanged(index(0, 0, mapFromSource(sourceIndex)), index(rowCount - 1, 0, mapFromSource(sourceIndex)));
}
...
/**
  ...
  @see setCheckedState
  */
bool CheckableProxyModel::setSourceIndexCheckedState(QModelIndex sourceIndex,
                                                     bool checked)
{
...
/**
  ...
  */
bool CheckableProxyModel::setCheckedState(QModelIndex proxyIndex,
                                          bool checked)
{
