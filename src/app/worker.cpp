...
{
public:
    WorkerPrivate(Worker *parent,
                  const QStringList &files,
                  QTextDocument *oldLicense,
                  QTextDocument *newLicense,
                  bool searchForOneLicense,
                  bool caseSensitive)
        : m_files(files)
...
void WorkerPrivate::init()
{
    ...
    for (int i = 0; i < data.length(); ++i) {
        ...
        if (ch == c_r) {
            ...
            if (!word.isEmpty()) {
                m_toReplace.append(Statement(Statement::Word, (m_caseSensitive ? word : word.toLower())));
...
Worker::Worker(const QStringList &files,
               QTextDocument *oldLicense,
               QTextDocument *newLicense,
               bool searchForOneLicense,
               bool caseSensitive,
        ...
        foreach (const QString &fileName, d->m_files) {
            ...
            if (!found.isEmpty()) {
                ...
                foreach (const LicensePos &p, found) {
                    data.replace(p.m_start, p.m_end - p.m_start, d->m_newLicense);
                }
