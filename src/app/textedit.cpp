...
void TextEditPrivate::init()
{
    ...
    q->document()->addResource(QTextDocument::ImageResource, QUrl(c_skipLine), QVariant(skipLine));

    QImage skipWord(QLatin1String(":/img/code-variable.png"));

    q->document()->addResource(QTextDocument::ImageResource, QUrl(c_skipWord), QVariant(skipWord));
}
...
void TextEdit::insertSkipLine()
{
    QTextCursor c = textCursor();
    c.insertImage(c_skipLine);
}

void TextEdit::insertSkipWord()
{
    QTextCursor c = textCursor();
